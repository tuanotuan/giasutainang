...
- Smart features: tutor matching, Zalo draft, class-post draft, tutor-profile audit, learning roadmap, operations report, and a multi-turn public quick-answer assistant using a chat-message Workers AI cascade (`@cf/openai/gpt-oss-20b`, GLM 4.7 Flash, then Llama 3.2) with bounded, de-duplicated output and topic-aware non-AI fallbacks.
...
- The tutor catalog intentionally starts empty after the owner-requested full deletion. Static tutor fallback data is empty, so deleted profiles cannot flash or reappear when the API is unavailable. New admin-created or application-approved profiles remain `unverified` until a real verification process occurs.
- `npm run export:tutors-pdf` exports all current production tutor profiles to a generic A4 PDF and stops with a clear message when the catalog is empty.
