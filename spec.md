...
- Tutor-request email notifications are implemented as a non-blocking background task after the D1 insert. Messages contain only a safe summary and admin link; the private destination remains in Cloudflare secret `NOTIFICATION_EMAIL`. Email Routing, verified destination, secret and `NOTIFY_EMAIL` binding are configured. Authenticated, rate-limited admin test email does not create a fake request.
...
- Current feature awaiting owner acceptance: Gmail notification for new parent tutor requests. Full private configuration and a mobile-friendly **Gửi email thử** control in admin are implemented; owner must confirm receipt of the test email, then submit one real-form test. Do not start another feature yet.
...
- Last updated: 2026-07-13 — private email configuration and authenticated admin test flow complete; live Gmail acceptance remains.
