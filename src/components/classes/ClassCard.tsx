...
export function ClassCard({ item }: { item: ClassItem }) {
  ...
  const detailHref = `/lop-moi/chi-tiet/?id=${encodeURIComponent(item.id)}`;
  return (
    <article className="rounded-2xl border border-slate-100 bg-white p-5 shadow-card transition hover:-translate-y-1 hover:border-primary-100 hover:shadow-soft">
      ...
      <div className="mt-5 border-t border-slate-100 pt-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Link href={detailHref} className="flex h-10 items-center justify-center rounded-xl border border-primary-200 text-xs font-bold text-primary-700 transition hover:bg-primary-50">
            Xem chi tiết
          </Link>
          <Link href={`${detailHref}#nhan-lop`} className={`flex h-10 items-center justify-center rounded-xl text-xs font-bold transition ${item.status === "assigned" ? "pointer-events-none bg-slate-100 text-slate-400" : "bg-primary-600 text-white hover:bg-primary-700"}`}>
