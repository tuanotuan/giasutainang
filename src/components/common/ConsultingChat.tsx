...
export function ConsultingChat() {
  ...
  return (
    <div className="fixed bottom-5 left-4 z-50 sm:bottom-7 sm:left-6">
      {open && (
        <section role="dialog" aria-label="Trợ lý tư vấn" className="mb-3 flex h-[min(560px,72vh)] w-[calc(100vw-2rem)] max-w-sm flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
      ...
      <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label={open ? "Đóng trợ lý tư vấn" : "Mở trợ lý tư vấn"} className="flex h-12 items-center gap-2 rounded-full bg-violet-600 px-4 text-xs font-bold text-white shadow-lg transition hover:scale-[1.02] hover:bg-violet-700"><MessageCircleQuestion className="h-5 w-5" /><span className="hidden sm:inline">Hỏi nhanh</span></button>
    </div>
  );
}
