...
export function ListingHero({ eyebrow, title, description }: ListingHeroProps) {
  return (
    <section className="relative overflow-hidden bg-primary-800 py-9 text-white sm:py-16">
      <div className="absolute -right-16 -top-32 h-80 w-80 rounded-full border-[52px] border-white/5" />
      <div className="container-page relative">
        <nav className="mb-4 flex min-w-0 items-center gap-2 text-xs text-primary-100 sm:mb-6" aria-label="Breadcrumb">
        ...
        <h1 className="max-w-3xl text-[28px] font-extrabold tracking-tight sm:text-4xl lg:text-5xl">{title}</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-primary-100 sm:mt-4 sm:text-base sm:leading-7">{description}</p>
      </div>
    </section>
  );
}
